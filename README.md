...
- **Administrador:** CRUD completo de productos con foto, una pestaña de **Agregar Crédito** para cargar a mano un crédito nuevo (Nombre Apoderado, Nombre Alumno(s) y Monto Crédito), un resumen con la cantidad y el monto total de transferencias sin usar, y una pestaña de **Recaudación** con el total recaudado por cada vendedor y día, separando efectivo de transferencia.
- **Vendedor:** registrar ventas seleccionando productos y cantidades (con subtotales y total automáticos), marcando si la venta es en **efectivo**, por **transferencia** o con un **crédito** (saldo a favor cargado de antes, que se puede ir usando de a poco en distintas compras). También tiene una pestaña de **Auditoría** con el total vendido en el día (efectivo, transferencia y crédito) y el detalle por producto, ambos limitados a sus propias ventas del día, y una pestaña de **Ventas del día** con todas las ventas del día en curso (de cualquier vendedor), de la más reciente a la más antigua, con el detalle de productos de cada una.
...
### Transferencias (dejó de usarse en el punto de venta)

Antes existía una pestaña donde el Vendedor buscaba una transferencia puntual en la hoja externa "registro" y la aplicaba a una sola venta (marcándola como **Usado**). Esa pestaña **se sacó de la app**: ahora todas las transferencias con saldo a favor se cargan directamente como **créditos** (ver más abajo), y desde Vender el Vendedor las usa desde ahí.

La hoja "registro" y la pestaña **Resumen** del Administrador (cuántas transferencias quedan sin usar y su monto total) siguen funcionando igual que antes, por si te sirven como registro de lo que va entrando; ya no están conectadas al flujo de venta.

### Créditos

Un **crédito** es un saldo a favor que se puede usar de a poco, en más de una compra (a diferencia de la vieja transferencia puntual, que se consumía entera de una vez). Vive en la misma hoja externa de transferencias, en dos pestañas propias:
...
Al igual que con transferencias, la cuenta de Google que despliega el Apps Script necesita acceso de edición a esa hoja externa. Nota: como valida el saldo contra el servidor, usar un crédito **requiere conexión** en el momento de registrar la venta (no funciona en modo sin señal). El descuento del saldo ubica la fila del crédito por su `id_credito` (que nunca cambia), no por la posición de la fila: si alguien inserta, borra o reordena filas a mano en la hoja CREDITO mientras un vendedor tiene la lista de créditos abierta en su celular, el descuento igual cae en el crédito correcto.

**Agregar Crédito (administrador):** en su pestaña **➕ Agregar Crédito**, el Administrador carga un crédito nuevo llenando **Nombre Apoderado**, **Nombre Alumno(s)** y **Monto Crédito**, y toca **Guardar crédito**. El servidor agrega una fila nueva en la hoja CREDITO con: `id_credito` correlativo (el número más alto que ya exista más 1), `Fecha` puesta por el servidor en ese momento, `Nombre completo` = Nombre Apoderado, `Nombre alumno` = Nombre Alumno(s), y tanto `Abono` como `Saldo` con el Monto Crédito ingresado (todavía no se usó nada). También queda registrado en `Usuario` qué administrador lo cargó. Las columnas Documento, Movimiento y RUN quedan vacías (son propias de las transferencias que llegan por otro medio) — se pueden completar a mano en la hoja si hace falta. Como escribe directo en la hoja, esta acción también requiere conexión.

Debajo del formulario, la misma pestaña **➕ Agregar Crédito** muestra la lista de **"Créditos con saldo disponible"** (los mismos que ve el Vendedor en su pestaña Créditos, más quién los cargó), para que el Administrador pueda confirmar de un vistazo que un crédito recién ingresado quedó bien registrado. Tiene su propio buscador (por nombre, alumno, ID, etc.) y un botón "🔄 Actualizar" para traer la lista más reciente.

**Editar o eliminar un crédito (administrador):** en esa misma lista, cada crédito tiene un botón **"Editar"** que abre una ventana para corregir el Nombre Apoderado, el Nombre Alumno(s) o el Monto Crédito, con un botón **Guardar** para confirmar los cambios. Si se corrige el Monto Crédito de un crédito que ya se usó en parte, el nuevo saldo se recalcula respetando lo ya gastado (no se "regala" de vuelta lo ya consumido). Esa misma ventana tiene un botón **Eliminar**: no borra la fila de la hoja CREDITO (se conserva como historial, con una columna `Estado` que se crea sola y queda en `FALSE`), pero el crédito deja de verse tanto en esta lista del Administrador como en la pestaña Créditos del Vendedor. Ambas acciones requieren conexión.
...
- **Tipo de venta:** en la pestaña Vender hay tres botones, **💵 Efectivo**, **💳 Transferencia** y **🏦 Crédito**. Por defecto queda en Efectivo. **💳 Transferencia** es una simple etiqueta: el vendedor la toca cuando el cliente pagó por transferencia directa (sin descontar nada de ninguna hoja), y así queda registrado en la columna `Tipo_venta`. **🏦 Crédito** en cambio sí queda ligado a un crédito puntual: se activa solo al elegir uno con el botón "Usar" desde la pestaña Créditos.
...
- **Auditoría (vendedor):** muestra, solo para el vendedor que tiene la sesión abierta, cuánto vendió hoy en efectivo, cuánto por transferencia, cuánto con crédito, y el detalle de cantidades por producto vendidas hoy. Se actualiza con el botón "🔄 Actualizar" o al abrir la pestaña.
...
- **Ventas:** si al tocar "Registrar venta" no hay conexión (o tarda demasiado), la venta queda guardada en el celular como pendiente — igual se puede imprimir el ticket en el momento, porque eso no depende de internet. Apenas vuelve la señal, la app manda solas todas las ventas pendientes a la Google Sheet (también se puede forzar con el botón "🔄 Sincronizar" que aparece en la barra de arriba cuando hay pendientes).
  - **Por qué a veces una venta "pendiente" ya está en la hoja:** si la señal es mala justo en el momento de tocar "Registrar venta", puede pasar que el pedido sí haya llegado y quedado guardado en el servidor, pero la respuesta se pierda en el camino de vuelta al celular. La app, al no recibir esa confirmación, la marca como "pendiente" igual — aunque en realidad ya está registrada. Esto es normal y **no genera una fila duplicada**: gracias al identificador único que lleva cada venta (columna `IDCliente`), cuando ese pendiente finalmente sincroniza, el servidor reconoce que ya la había recibido y sólo confirma el mismo registro, sin crear uno nuevo.
- **Productos:** quedan guardados en el celular después de la primera carga, así que la pantalla de Vender sigue funcionando sin señal.
  - **Ojo con los cambios de precio:** como los productos se cargan una sola vez al iniciar sesión, si el Administrador cambia un precio mientras un Vendedor ya tiene la app abierta, ese Vendedor va a seguir viendo el precio viejo hasta que haga algo para refrescarlo. En la pestaña **Vender** hay un botón **"🔄 Actualizar productos"** para eso — tocándolo alcanza para traer los precios y productos más recientes, sin necesidad de cerrar sesión ni, mucho menos, de borrar los datos de navegación del celular. **Nunca conviene borrar los datos de navegación (cookies y datos de sitios) de la app si hay ventas pendientes de sincronizar** (ver más arriba): esas ventas viven únicamente en el celular hasta que logran subir a la Google Sheet, y borrar los datos de navegación las borra a ellas también, sin posibilidad de recuperarlas.
- **Créditos:** la app guarda una copia de los créditos con saldo disponible cada vez que hay señal (al entrar como vendedor y al abrir la pestaña Créditos). Si se abre esa pestaña sin conexión, muestra esa copia guardada con un cartel amarillo avisando que puede estar desactualizada — pero **usar un crédito en una venta sí requiere señal en el momento de registrarla**, porque el saldo se valida y descuenta contra el servidor.

**Límite importante a tener en cuenta:** si dos vendedores usan el mismo crédito casi al mismo tiempo, el servidor procesa las ventas una por una (nunca en paralelo) así que no se puede pasar del saldo disponible entre las dos; la segunda venta que supere lo que quedaba después de la primera va a ser rechazada, avisando que el saldo ya no alcanza.
...
4. Antes de imprimir por primera vez: tocá **🖨️ Impresora** (arriba), encendé/emparejá tu impresora térmica Bluetooth y seleccionala en la lista que aparece. Mientras no esté conectada (por ejemplo, recién entraste a la app o la impresora se apagó), el botón muestra un aviso ⚠️; desaparece apenas se conecta. Esto es sólo un indicador — no impide vender ni imprimir por la alternativa de "Compartir" (ver más abajo).
   ...
   - **Al actualizar la página:** recargar la página reinicia todo (incluida la conexión Bluetooth activa), pero Chrome recuerda el permiso que se le dio a la impresora. Gracias a eso, apenas carga la página la app intenta reconectarse sola a la última impresora usada, sin volver a pedir nada — no hace falta tocar "🖨️ Impresora" de nuevo salvo que la impresora esté apagada o fuera de alcance en ese momento.
